					...
					<table class="table table-sm table-hover align-middle" border="1">
						...
						<tr>
							<td><a class="bi bi-link-45deg"
								href="<%=request.getContextPath()%>/profil?id=${sessionScope.isConnected.noUtilisateur}">${sessionScope.isConnected.noUtilisateur}</a>
							</td>
							...
							<td class="d-none d-md-table-cell">${sessionScope.isConnected.rue},</br>${sessionScope.isConnected.codePostal}
								${sessionScope.isConnected.ville}
							</td>
							<td class="d-none d-md-table-cell">${sessionScope.isConnected.credit}</td>
							<td><c:choose>
									<c:when
										test="${sessionScope.isConnected.administrateur eq true}">
										<img
											src="${pageContext.request.contextPath}/img/administrateur.png"
											width="20" height="20" alt="icone administrateur">
									</c:when>
								</c:choose></td>
							<td></td>
						</tr>
						<c:forEach var="u" items="${listeUtilisateurs}">
							<c:if
								test="${u.noUtilisateur != sessionScope.isConnected.noUtilisateur}">
								<tr>
									<td><a class="bi bi-link-45deg"
										href="<%=request.getContextPath()%>/profil?id=${u.noUtilisateur}">${u.noUtilisateur}</a>
									</td>
									<td>${u.pseudo}</td>
									...
									<td class="d-none d-md-table-cell">${u.rue},</br>${u.codePostal}
										${u.ville}
									</td>
									...
									<td><c:if test="${not u.administrateur}">
											<div class="form-check form-switch">
												<form id="checkActif_${u.noUtilisateur}"
													action="<%=request.getContextPath()%>/actif?id=${u.noUtilisateur}"
													method="post">
													<input data-user-id="${u.noUtilisateur}"
									...
									<c:choose>
										<c:when test="${not u.administrateur}">
											<td><a
												href="${pageContext.request.contextPath}/supprimer?id=${u.noUtilisateur}"
