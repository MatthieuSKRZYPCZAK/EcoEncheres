...
<body>
	...
	<main>
	<c:choose>
		<c:when test="${sessionScope.isConnected.noUtilisateur eq utilisateur.noUtilisateur }">
		...
		<div class="row justify-content-center text-center">
			...
			<div class="container mt-5">
				<table class="table table-sm table-hover align-middle table-striped" border="1">
					...
					<c:forEach var="e" items="${listeEncheres}">
							<tr>
								<td>
									<a class="bi bi-link-45deg" href="<%=request.getContextPath()%>/article?id=${e.noArticle}">${e.noArticle}</a>
								</td>
								<td>${e.nomArticle}</td>
 								<td>${e.acheteur.pseudo}</td>
								<td>${e.prixInitial}</td>
								<td>${e.prixVente}</td>
								<td class="d-none d-md-table-cell"><fmt:formatDate pattern = "dd-MM-yyyy HH:mm" 
													value = "${e.dateDebutEncheres}" />
								</td>
								<td class="d-none d-md-table-cell"><fmt:formatDate pattern = "dd-MM-yyyy HH:mm" 
													value = "${e.dateFinEncheres}" />
								</td>
								<td>${e.etatVente}</td>
								...
								<td>
								<c:if test="${e.etatVente eq 'créé'}">
									<a href="${pageContext.request.contextPath}/supprimerVente?id=${e.noArticle}&ventes=ventes" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette vente ?');">
